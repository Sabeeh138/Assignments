...
def rnn_step_forward(x, prev_h, Wx, Wh, b):
    ...
    """
    h_next, cache = None, None
    ##########################################################################
    # Single step of RNN forward pass
    h_next = torch.tanh(x @ Wx + prev_h @ Wh + b)
    cache = (x, prev_h, Wx, Wh, h_next)
    ##########################################################################
    #                             END OF YOUR CODE                           #
    ##########################################################################
    return h_next, cache
...
def rnn_step_backward(dnext_h, cache):
    ...
    ##########################################################################
    # Single step of RNN backward pass
    x, prev_h, Wx, Wh, h_next = cache
    grad_tanh = dnext_h * (1 - h_next * h_next) 
    
    dx = grad_tanh @ Wx.t() 
    dprev_h = grad_tanh @ Wh.t() 
    dWx = x.t() @ grad_tanh 
    dWh = prev_h.t() @ grad_tanh 
    db = grad_tanh.sum(dim=0) 
    ##########################################################################
    #                             END OF YOUR CODE                           #
    ##########################################################################
...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    h, cache = None, None
    ##########################################################################
    # Vanilla RNN forward pass over sequence
    N, T, D = x.size()
    H = h0.size(1)
    h = x.new_zeros(N, T, H)
    cache = []
    h_prev = h0
    for t in range(T):
        h_t, step_cache = rnn_step_forward(x[:, t, :], h_prev, Wx, Wh, b)
        h[:, t, :] = h_t
        h_prev = h_t
...
def rnn_backward(dh, cache):
    ...
    ##########################################################################
    # Vanilla RNN backward pass over sequence
    x_sample, _, Wx, Wh, _ = cache[0]
    N, T, H = dh.size()
    D = x_sample.size(1)
    ...
    dh_prev = torch.zeros((N, H), device=dh.device, dtype=dh.dtype)
    
    for t in reversed(range(T)):
        grad_h_t = dh[:, t, :] + dh_prev
        dx_t, dh_prev, dWx_t, dWh_t, db_t = rnn_step_backward(grad_h_t, cache[t])
        dx[:, t, :] = dx_t
        dWx += dWx_t
        dWh += dWh_t
        db += db_t
...
def temporal_softmax_loss(x, y, ignore_index=None):
    ...
    loss = F.cross_entropy(x.reshape(-1, x.shape[-1]), y.reshape(-1), ignore_index=ignore_index if ignore_index is not None else -100, reduction='sum') / x.shape[0]
    ##########################################################################
    #                             END OF YOUR CODE                           #
    ##########################################################################
...
class CaptioningRNN(nn.Module):
    ...
    def forward(self, images, captions):
        ...
        if self.cell_type == 'attn':
            ...
            h = self.cell(self.word_embedding(captions_in), A)
        else:
            h0 = self.feature_projection(features.mean(dim=(2, 3)))
            h = self.cell(self.word_embedding(captions_in), h0)
    ...
    def sample(self, images, max_length=15):
        ...
        curr_word = torch.full((N,), self._start, device=images.device, dtype=torch.long)
        for t in range(max_length):
            x = self.word_embedding(curr_word.unsqueeze(1)).squeeze(1)
            if self.cell_type == 'rnn':
                h = self.cell.step_forward(x, h)
            elif self.cell_type == 'lstm':
                h, c = self.cell.step_forward(x, h, c)
            elif self.cell_type == 'attn':
                attn_emb, attn_weights = dot_product_attention(h, A)
                attn_weights_all[:, t, :, :] = attn_weights
                h, c = self.cell.step_forward(x, h, c, attn_emb)
...
class LSTM(nn.Module):
    ...
    def step_forward(
        ...
        """
        # Linear transformation for all gates
        all_gate_inputs = x @ self.Wx + prev_h @ self.Wh + self.b
        
        # Split gates into i, f, o, g
        i_gate, f_gate, o_gate, g_gate = torch.split(all_gate_inputs, prev_h.size(1), dim=1)
...
def dot_product_attention(prev_h, A):
    ...
    """
    N, H, H_feat, W_feat = A.shape

    # Flatten spatial features
    A_reshaped = A.reshape(N, H, -1) # (N, H, 16)
    
    # Compute attention scores
    # (N, 1, H) @ (N, H, 16) -> (N, 1, 16)
    attn_scores = torch.matmul(prev_h.unsqueeze(1), A_reshaped)
    attn_scores = attn_scores.squeeze(1) / math.sqrt(H)
    
    # Softmax to get weights
    weights = F.softmax(attn_scores, dim=1) # (N, 16)
    
    # Compute weighted sum of features
    # (N, H, 16) @ (N, 16, 1) -> (N, H, 1)
    context = torch.matmul(A_reshaped, weights.unsqueeze(2)).squeeze(2) # (N, H)
    
    # Reshape weights back to spatial grid
    weights_spatial = weights.view(N, H_feat, W_feat)
    ##########################################################################
    #                             END OF YOUR CODE                           #
    ##########################################################################

    return context, weights_spatial
...
class AttentionLSTM(nn.Module):
    ...
    def step_forward(
        ...
        H = prev_h.shape[1]
        gates = x.mm(self.Wx) + prev_h.mm(self.Wh) + attn.mm(self.Wattn) + self.b
        i = torch.sigmoid(gates[:, 0:H])
        f = torch.sigmoid(gates[:, H:2*H])
        o = torch.sigmoid(gates[:, 2*H:3*H])
        g = torch.tanh(gates[:, 3*H:4*H])
    ...
    def forward(self, x: torch.Tensor, A: torch.Tensor):
        ...
        prev_c = c0
        for t in range(T):
            attn_emb, _ = dot_product_attention(prev_h, A)
            next_h, next_c = self.step_forward(x[:, t, :], prev_h, prev_c, attn_emb)
            hn[:, t, :] = next_h
            prev_h = next_h
            prev_c = next_c
        ######################################################################
